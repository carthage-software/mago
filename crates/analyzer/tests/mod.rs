...
// test_case!(classes_property_assignment_typed_null); // FIXME: missing file, registration left by dead agent
...
// test_case!(flow_complex_match_subjects); // FN: match-true with enum equality not recognised exhaustive
test_case!(flow_double_isset_short_circuit);
// test_case!(flow_double_null_check); // FN: redundant `=== null` after null branch returned
...
// test_case!(flow_low_prec_and); // FN: `and` operator does not narrow like `&&`
// test_case!(flow_low_prec_or); // FN: `or` operator does not narrow like `||`
test_case!(flow_match_true_redundant_last_arm);
// test_case!(flow_match_arm_types); // FN: match-true with instanceof not recognised exhaustive
...
// test_case!(flow_method_on_null_narrow); // FN: $s = null; $s->run() reports only mixed-return, not method-access-on-null
...
// test_case!(flow_redundant_instanceof); // FN: `$s instanceof T` for typed `T $s` not flagged redundant
...
fn test_all_test_cases_are_ran() {
    ...
    for entry in test_cases_dir {
        ...
        // Files with `_FP_` / `_FN_` in the name are intentionally unregistered:
        // they document false-positive / false-negative findings against current
        // analyzer behaviour and are not expected to pass under the test
        // framework. They live alongside the regular cases for triage.
        if file_name.contains("_FP_") || file_name.contains("_FN_") {
            continue;
        }
...
// Late-registered tests (orphaned files from terminated agents)
