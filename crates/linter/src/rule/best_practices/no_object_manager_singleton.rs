...
impl LintRule for NoObjectManagerSingletonRule {
        ...
        A: Arena,
    {
        ...
        if !method_id.value.eq_ignore_ascii_case(b"getInstance") {
            return;
        }
        ...
        if !fqcn.eq_ignore_ascii_case(b"Magento\\Framework\\App\\ObjectManager") {
            return;
        }

        ctx.collector.report(
            Issue::new(self.cfg.level(), "Direct usage of `ObjectManager::getInstance()` is discouraged.")
                .with_code(self.meta.code)
                .with_annotation(
                    Annotation::primary(call.span()).with_message("Use constructor dependency injection instead"),
                )
