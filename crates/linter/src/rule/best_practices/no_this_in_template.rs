...
impl LintRule for NoThisInTemplateRule {
        ...
        A: Arena,
    {
        ...
        if !ctx.source_file.name.ends_with(b".phtml") {
            return;
        }

        ctx.collector.report(
            Issue::new(self.cfg.level(), "The use of `$this` in templates is deprecated.")
                .with_code(self.meta.code)
                .with_annotation(Annotation::primary(var.span()).with_message("Use `$block` instead of `$this`"))
