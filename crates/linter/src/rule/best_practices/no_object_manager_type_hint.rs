...
impl LintRule for NoObjectManagerTypeHintRule {
        ...
        A: Arena,
    {
        ...
        if !fqcn.eq_ignore_ascii_case(b"Magento\\Framework\\ObjectManagerInterface") {
            return;
        }

        ctx.collector.report(
            Issue::new(self.cfg.level(), "Injecting `ObjectManagerInterface` directly is discouraged.")
