    ...
    A: Arena,
{
    ...
    ) -> Option<U16Flags<AttributeTarget>> {
        if is_attribute_name(class_name) {
            let mut flags = U16Flags::new();
    ...
    fn fold_attribute_target(&self, expression: &cst::Expression<'_>) -> Option<U16Flags<AttributeTarget>> {
    ...
    fn attribute_constant_target(&self, access: &cst::ClassConstantAccess<'_>) -> Option<U16Flags<AttributeTarget>> {
...
fn all_targets() -> U16Flags<AttributeTarget> {
    let mut flags = U16Flags::new();
    flags.set(AttributeTarget::Class);
...
fn all_targets_repeatable() -> U16Flags<AttributeTarget> {
