...
impl<'arena> NamespaceResolution<'arena> {
    ...
    pub(crate) fn resolve_name(&self, kind: NameResolutionKind, name: &'arena [u8]) -> &'arena [u8] {
        if let [b'\\', rest @ ..] = name {
            return rest;
        }
    ...
    #[must_use]
    pub fn lookup_alias(&self, kind: NameResolutionKind, query: &[u8]) -> Option<&'arena [u8]> {
    ...
    fn add_use_alias(&mut self, kind: NameResolutionKind, item: &'arena cst::UseItem<'arena>) {
    ...
    fn push_alias(&mut self, kind: NameResolutionKind, alias: &'arena [u8], fully_qualified_name: &'arena [u8]) {
        match kind {
