...
pub(crate) struct VariableBindings<'scratch, 'arena, S>
where
    S: Arena,
{
    pub(crate) unnamed: Option<&'arena TypeAnnotation<'arena>>,
    pub(crate) named:
        Vec<'scratch, (DirectVariable<'arena>, &'arena TypeAnnotation<'arena>, Option<TagVendor>, Span), S>,
}

impl<'arena, S> VariableBindings<'_, 'arena, S>
where
    S: Arena,
{
    ...
    A: Arena,
{
    pub(crate) fn collect_var_bindings(
    ...
    ) -> VariableBindings<'scratch, 'arena, S> {
        let mut named = Vec::new_in(self.scratch);
        ...
        for element in document.elements.iter() {
            ...
            match value.variable {
                Some(variable) => {
                    let variable = self.phpdoc_variable(&variable);
