...
fn first_version_argument<'arena>(argument_list: &'arena cst::ArgumentList<'arena>) -> Option<PHPVersion> {
...
fn push_since(ranges: &mut Vec<'_, PHPVersionRange>, version: PHPVersion) {
    ranges.push(PHPVersionRange::from(version));
}

fn push_until(ranges: &mut Vec<'_, PHPVersionRange>, version: PHPVersion) {
