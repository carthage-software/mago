...
use crate::metadata::function_like::TemplateTuple;
...
struct MethodUpdate {
    child_method_id: (Atom, Atom),
    return_type: Option<TypeMetadata>,
    param_updates: Vec<(usize, TypeMetadata)>,
    template_types: Option<Vec<TemplateTuple>>,
    thrown_types: Option<Vec<TypeMetadata>>,
    assertions: Option<BTreeMap<Atom, Vec<Assertion>>>,
    if_true_assertions: Option<BTreeMap<Atom, Vec<Assertion>>>,
    if_false_assertions: Option<BTreeMap<Atom, Vec<Assertion>>>,
}
...
pub fn inherit_method_docblocks(codebase: &mut CodebaseMetadata) {
    let work_items = collect_work_items(codebase);

    if work_items.is_empty() {
        return;
    }

    let max_depth = work_items.iter().map(|w| w.depth).max().unwrap_or(0);
    let mut work_by_level: Vec<Vec<InheritanceWork>> = vec![Vec::new(); max_depth + 1];

    for work in work_items {
        work_by_level[work.depth].push(work);
    }

    for level_work in work_by_level {
        if level_work.is_empty() {
            continue;
        }

        let codebase_ref: &CodebaseMetadata = &*codebase;

        let updates: Vec<MethodUpdate> =
            level_work.into_par_iter().filter_map(|work| compute_inheritance(&work, codebase_ref)).collect();

        for update in updates {
            apply_update(codebase, update);
        }
    }
}

/// Collect all work items for docblock inheritance.
fn collect_work_items(codebase: &CodebaseMetadata) -> Vec<InheritanceWork> {
    let codebase_ref: &CodebaseMetadata = codebase;

    codebase_ref
        .class_likes
        .par_iter()
        .flat_map_iter(|(class_name, class_metadata)| {
            let depth = class_metadata.all_parent_classes.len() + class_metadata.all_parent_interfaces.len();

            class_metadata.overridden_method_ids.iter().filter_map(move |(method_name, method_ids)| {
                ...
                parent_method_id.map(|(parent_class, parent_method)| InheritanceWork {
...
fn compute_inheritance(work: &InheritanceWork, codebase: &CodebaseMetadata) -> Option<MethodUpdate> {
    let child_method_id = (work.class_name, work.method_name);
    let parent_method_id = (work.parent_class, work.parent_method);

    let parent_method = codebase.function_likes.get(&parent_method_id)?;
    let child_class = codebase.class_likes.get(&work.class_name)?;
    let child_method = codebase.function_likes.get(&child_method_id)?;
    ...
    let template_map = child_class.template_extended_parameters.get(&work.parent_class);
    ...
    let template_result = template_map.map(|template_map| {
        ...
        for (template_name, concrete_type) in template_map {
            template_result.add_lower_bound(
                *template_name,
                GenericParent::ClassLike(work.parent_class),
                concrete_type.clone(),
            );
        }
        template_result
    ...
    let return_type_update = if should_inherit_return {
        substituted_return_type.map(|(type_union, span, from_docblock)| {
            ...
            TypeMetadata { type_union: narrowed_type, span, from_docblock, inferred: false }
        })
    } else {
        None
    };

    let param_updates: Vec<(usize, TypeMetadata)> = substituted_param_types
        .into_iter()
        .enumerate()
        .filter_map(|(i, substituted_param)| {
            if params_to_inherit.get(i).copied() == Some(true)
                && let Some((type_union, span, from_docblock)) = substituted_param
            {
                Some((i, TypeMetadata { type_union, span, from_docblock, inferred: false }))
            } else {
                None
            }
        })
    ...
    if return_type_update.is_none()
        && param_updates.is_empty()
    ...
    Some(MethodUpdate {
        child_method_id,
        return_type: return_type_update,
        param_updates,
        template_types: if should_inherit_templates { Some(parent_template_types.clone()) } else { None },
        thrown_types: if should_inherit_thrown { Some(substituted_thrown_types) } else { None },
        assertions: if should_inherit_assertions { Some(resolve_assertions(parent_assertions)) } else { None },
        if_true_assertions: if should_inherit_if_true_assertions {
            Some(resolve_assertions(parent_if_true_assertions))
        } else {
            None
        },
        if_false_assertions: if should_inherit_if_false_assertions {
...
fn apply_update(codebase: &mut CodebaseMetadata, update: MethodUpdate) {
    let Some(child_method) = codebase.function_likes.get_mut(&update.child_method_id) else {
        return;
    };

    if let Some(return_type) = update.return_type {
        child_method.return_type_metadata = Some(return_type);
    }

    for (i, type_metadata) in update.param_updates {
        if let Some(child_param) = child_method.parameters.get_mut(i) {
            child_param.type_metadata = Some(type_metadata);
        }
    }

    if let Some(template_types) = update.template_types {
        child_method.template_types = template_types;
    }

    if let Some(thrown_types) = update.thrown_types {
        child_method.thrown_types = thrown_types;
    }

    if let Some(assertions) = update.assertions {
        child_method.assertions = assertions;
    }

    if let Some(if_true_assertions) = update.if_true_assertions {
        child_method.if_true_assertions = if_true_assertions;
    }

    if let Some(if_false_assertions) = update.if_false_assertions {
        child_method.if_false_assertions = if_false_assertions;
    }
}
