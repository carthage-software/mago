...
impl CodebaseMetadata {
    ...
    fn compute_inherited_methods_for_patch(&self, k: &Word, patch: &ClassLikeMetadata) -> WordSet {
        let Some(existing) = self.class_likes.get(k) else {
            return WordSet::default();
        };
        if existing.flags.is_user_defined() || patch.methods.is_empty() {
            return WordSet::default();
        }
        let ancestor_methods = class_like::collect_ancestor_methods(existing, &self.class_likes);
    ...
    /// a full clone of the source metadata's outer HashMap/WordMap structures.
    ///
    ...
    pub fn extend_ref(&mut self, other: &CodebaseMetadata) {
        ...
        self.file_signatures.extend(other.file_signatures.iter().map(|(&k, v)| (k, v.clone())));
    ...
    /// Applies patch overrides from a patch codebase into this one.
    ///
    ...
    ///
    /// Must be called after all non-patch partials have been merged so that the slots patches
    /// target are already present.
    pub fn apply_patches(&mut self, patches: CodebaseMetadata) {
        let mut orphan_patch_classes = WordSet::default();

        for (k, v) in patches.class_likes {
        ...
        for (k, v) in patches.function_likes {
            match self.function_likes.entry(k) {
                Entry::Occupied(mut entry) => {
        ...
        self.symbols.extend_except(patches.symbols, &orphan_patch_classes);

        for (k, v) in patches.all_class_like_descendants {
            self.all_class_like_descendants
                .entry(k)
                .or_default()
                .extend(v.into_iter().filter(|d| !orphan_patch_classes.contains(d)));
        }

        for (k, v) in patches.direct_classlike_descendants {
            self.direct_classlike_descendants
                .entry(k)
                .or_default()
                .extend(v.into_iter().filter(|d| !orphan_patch_classes.contains(d)));
        }

        self.file_signatures.extend(patches.file_signatures);
        self.safe_symbols.extend(patches.safe_symbols);
        self.safe_symbol_members.extend(patches.safe_symbol_members);
        self.infer_types_from_usage |= patches.infer_types_from_usage;
        self.patch_diagnostics.extend(patches.patch_diagnostics);
    }

    /// Applies patch overrides from a patch codebase by reference, cloning only
    ...
    ///
    /// Must be called after all non-patch partials have been merged via so that the slots patches
    /// target are already present.
    pub fn apply_patches_ref(&mut self, patches: &CodebaseMetadata) {
        let mut orphan_patch_classes = WordSet::default();
        ...
        self.symbols.extend_ref_except(&patches.symbols, &orphan_patch_classes);

        for (k, v) in &patches.all_class_like_descendants {
            self.all_class_like_descendants
                .entry(*k)
                .or_default()
                .extend(v.iter().filter(|d| !orphan_patch_classes.contains(*d)).copied());
        }
        ...
        self.file_signatures.extend(patches.file_signatures.iter().map(|(&k, v)| (k, v.clone())));
        self.safe_symbols.extend(patches.safe_symbols.iter().copied());
        self.safe_symbol_members.extend(patches.safe_symbol_members.iter().copied());
        self.infer_types_from_usage |= patches.infer_types_from_usage;
        self.patch_diagnostics.extend(patches.patch_diagnostics.iter().cloned());
    }
...
#[inline]
fn is_free_function_id(k: &(Word, Word)) -> bool {
    k.0.is_empty()
}
