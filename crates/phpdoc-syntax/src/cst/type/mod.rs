...
pub enum Type<'arena> {
    ...
    ClassLikeString(ClassLikeStringType<'arena>),
...
impl HasSpan for Type<'_> {
    fn span(&self) -> Span {
        match self {
            ...
            Type::ClassLikeString(ty) => ty.span(),
...
impl std::fmt::Display for Type<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Type::ClassLikeString(ty) => write!(f, "{ty}"),
            ...
            Type::ThisVariable(ty) => write!(f, "{ty}"),
