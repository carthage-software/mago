...
#[test]
fn list_in_value_position_is_never() {
    assert_eq!(ty(b"<?php list($a, $b);"), "never");
}

#[test]
fn throw_is_never() {
    assert_eq!(ty(b"<?php throw $e;"), "never");
}

#[test]
fn clone_keeps_objects_and_rejects_scalars() {
    assert_eq!(ty(b"<?php clone $x;"), "mixed");
    assert_eq!(ty(b"<?php $x = 1; clone $x;"), "never");
}

#[test]
fn keywords_in_value_position_are_never() {
    assert_eq!(ty(b"<?php self;"), "never");
    assert_eq!(ty(b"<?php parent;"), "never");
}
