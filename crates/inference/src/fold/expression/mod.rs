    ...
    A: Arena,
{
    pub fn infer_expression(
        &mut self,
        expression: &Expression<'source, SymbolId, S, E>,
        ...
        let mut typed = match &expression.kind {
            ...
            ExpressionKind::Include(operand) => {
                self.infer_require_like(expression.span, operand, RequireKind::Include)?
            }
            ExpressionKind::IncludeOnce(operand) => {
                self.infer_require_like(expression.span, operand, RequireKind::IncludeOnce)?
            }
            ExpressionKind::Require(operand) => {
                self.infer_require_like(expression.span, operand, RequireKind::Require)?
            }
            ...
            ExpressionKind::MagicConstant(magic_constant) => {
