...
where
    A: Arena,
{
    pub(crate) fn infer_class(
        &mut self,
        symbol: SymbolId,
        node: &'source Class<'source, SymbolId, S, E>,
    ) -> InferenceResult<Class<'arena, SymbolId, Flow, Type<'arena>>> {
        ...
        Ok(Class {
            ...
            attribute_target: node.attribute_target,
