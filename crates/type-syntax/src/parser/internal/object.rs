...
pub fn parse_object_type<'input>(stream: &mut TypeTokenStream<'input>) -> Result<Type<'input>, ParseError> {
    ...
    let (keyword, kind) = match next.kind {
        TypeTokenKind::Object => {
            let keyword = Keyword::from(stream.consume()?);
            if !stream.is_at(TypeTokenKind::LeftBrace)? {
                return Ok(Type::Object(ObjectType { keyword, parameters: None }));
            }
            (keyword, ShapeTypeKind::Object)
        }
        _ => {
            return Err(ParseError::UnexpectedToken(vec![TypeTokenKind::Object], next.kind, next.span));
        }
    };
