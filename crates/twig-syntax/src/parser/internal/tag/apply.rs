...
impl<'arena> Parser<'_, 'arena> {
    pub(crate) fn parse_apply(
        ...
        let first_name = self.expect_identifier(b"expected filter name after `apply`")?;
        ...
        let body = self.parse_statements(&BlockTerminator { names: &[b"endapply"] })?;
        let end_open_tok = self.stream.expect_block_start()?;
        let end_open_tag = self.stream.span_of(&end_open_tok);
        let end_kw_tok = self.stream.expect_name(b"expected `endapply`")?;
        if end_kw_tok.value != b"endapply" {
            return Err(ParseError::MismatchedEndTag {
                expected: b"endapply".to_vec(),
