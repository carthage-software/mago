...
impl MutWalker<Context<'_, '_>> for ModuleBuildingWalker {
    #[inline]
    fn walk_in_statement(&mut self, statement: &Statement, context: &mut Context<'_, '_>) {
        context.ancestors.push(statement.span());
    }

    #[inline]
    fn walk_in_expression(&mut self, expression: &Expression, context: &mut Context<'_, '_>) {
        context.ancestors.push(expression.span());
    }

    #[inline]
    fn walk_out_statement(&mut self, _statement: &Statement, context: &mut Context<'_, '_>) {
        context.ancestors.pop();
    }

    #[inline]
    fn walk_out_expression(&mut self, _expression: &Expression, context: &mut Context<'_, '_>) {
        context.ancestors.pop();
    }

    #[inline]
    fn walk_in_program(&mut self, program: &Program, context: &mut Context<'_, '_>) {
        checker::statement::check_top_level_statements(program, context);
    }

    #[inline]
    fn walk_opening_tag(&mut self, opening_tag: &OpeningTag, context: &mut Context<'_, '_>) {
        checker::statement::check_opening_tag(opening_tag, context);
    }

    #[inline]
    fn walk_in_declare(&mut self, declare: &Declare, context: &mut Context<'_, '_>) {
        checker::statement::check_declare(declare, context);
    }

    #[inline]
    fn walk_in_namespace(&mut self, namespace: &Namespace, context: &mut Context<'_, '_>) {
        checker::statement::check_namespace(namespace, context);
    }

    #[inline]
    fn walk_in_hint(&mut self, hint: &Hint, context: &mut Context<'_, '_>) {
        context.hint_depth += 1;
        checker::hint::check_hint(hint, context);
    }

    #[inline]
    fn walk_out_hint(&mut self, _hint: &Hint, context: &mut Context<'_, '_>) {
        context.hint_depth -= 1;
    }

    #[inline]
    fn walk_in_try(&mut self, r#try: &Try, context: &mut Context<'_, '_>) {
    ...
    #[inline]
    fn walk_in_attribute_list(&mut self, attribute_list: &AttributeList, context: &mut Context<'_, '_>) {
        checker::attribute::check_attribute_list(attribute_list, context);
    }

    #[inline]
    fn walk_in_goto(&mut self, goto: &Goto, context: &mut Context<'_, '_>) {
        checker::statement::check_goto(goto, context);
    }

    #[inline]
    fn walk_in_argument_list(&mut self, argument_list: &ArgumentList, context: &mut Context<'_, '_>) {
    ...
    #[inline]
    fn walk_in_match(&mut self, r#match: &Match, context: &mut Context<'_, '_>) {
        checker::control_flow::check_match(r#match, context);
    }

    #[inline]
    fn walk_in_switch(&mut self, switch: &Switch, context: &mut Context<'_, '_>) {
        checker::control_flow::check_switch(switch, context);
    }

    #[inline]
    fn walk_in_assignment(&mut self, assignment: &Assignment, context: &mut Context<'_, '_>) {
    ...
    #[inline]
    fn walk_in_closure_creation(&mut self, closure_creation: &ClosureCreation, context: &mut Context<'_, '_>) {
        checker::closure_creation::check_closure_creation(closure_creation, context);
    }

    #[inline]
    fn walk_in_list(&mut self, list: &List, context: &mut Context<'_, '_>) {
        checker::array::check_list(list, context);
    }

    fn walk_in_call(&mut self, call: &Call, context: &mut Context<'_, '_>) {
    ...
    #[inline]
    fn walk_in_access(&mut self, access: &Access, context: &mut Context<'_, '_>) {
    ...
    #[inline]
    fn walk_literal_expression(&mut self, literal_expression: &Literal, context: &mut Context<'_, '_>) {
