...
impl<'arena> BinaryOperator<'arena> {
    ...
    pub fn as_bytes(&self) -> &'arena [u8] {
        match self {
            Self::Addition(_) => b"+",
            Self::Subtraction(_) => b"-",
            Self::Multiplication(_) => b"*",
            Self::Division(_) => b"/",
            Self::Modulo(_) => b"%",
            Self::Exponentiation(_) => b"**",
            Self::BitwiseAnd(_) => b"&",
            Self::BitwiseOr(_) => b"|",
            Self::BitwiseXor(_) => b"^",
            Self::LeftShift(_) => b"<<",
            Self::RightShift(_) => b">>",
            Self::NullCoalesce(_) => b"??",
            Self::Equal(_) => b"==",
            Self::NotEqual(_) => b"!=",
            Self::Identical(_) => b"===",
            Self::NotIdentical(_) => b"!==",
            Self::AngledNotEqual(_) => b"<>",
            Self::LessThan(_) => b"<",
            Self::LessThanOrEqual(_) => b"<=",
            Self::GreaterThan(_) => b">",
            Self::GreaterThanOrEqual(_) => b">=",
            Self::Spaceship(_) => b"<=>",
            Self::StringConcat(_) => b".",
            Self::And(_) => b"&&",
            Self::Or(_) => b"||",
