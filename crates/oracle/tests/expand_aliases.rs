...
fn alias_to_union_flat_merges() {
    fixture(|f| {
        let symbols = symbol_table(f.arena, "<?php /** @type Key = int|string */ class Foo {}");
...
fn alias_inside_object_type_args_expands() {
    fixture(|f| {
        ...
        let result = expand::expand(generic, &symbols, &mut f.builder);
...
fn alias_inside_keyed_array_value_expands() {
    fixture(|f| {
        ...
        let result = expand::expand(shape, &symbols, &mut f.builder);
...
fn no_alias_no_change() {
    fixture(|f| {
        let symbols = empty_symbol_table(f.arena);
        assert_eq!(expand::expand(well_known::TYPE_INT, &symbols, &mut f.builder), well_known::TYPE_INT);
        assert_eq!(
            expand::expand(well_known::TYPE_INT_OR_STRING, &symbols, &mut f.builder),
...
fn expanded_handle_is_stable() {
    fixture(|f| {
        ...
        let alias = t_alias(f, "Foo", "Id");
        let ty = f.u(alias);
        let first = expand::expand(ty, &symbols, &mut f.builder);
        let second = expand::expand(ty, &symbols, &mut f.builder);
...
fn deeply_nested_alias_in_box_of_list_expands() {
    fixture(|f| {
        ...
        let result = expand::expand(box_of_list, &symbols, &mut f.builder);
