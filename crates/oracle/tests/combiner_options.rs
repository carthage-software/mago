...
fn overwrite_empty_array_drops_when_other_array_present() {
    fixture(|f| {
        ...
        let out = join::compute_with(&[f.t_empty_array(), other], &options, &mut f.builder);
        assert_eq!(out.len(), 1);
...
fn overwrite_empty_array_keeps_when_alone() {
    fixture(|f| {
        let options = JoinOptions::structural().with_overwrite_empty_array(true);
        let out = join::compute_with(&[f.t_empty_array()], &options, &mut f.builder);
...
fn string_literal_collapse_fires_above_threshold() {
    fixture(|f| {
        ...
        let out = join::compute_with(&literals, &options, &mut f.builder);
...
fn merge_int_ranges_combines_overlapping_ranges() {
    fixture(|f| {
        ...
        let out = join::compute_with(&[zero_to_ten, five_to_fifteen], &options, &mut f.builder);
...
fn rewrite_int_keyed_to_list_converts_contiguous_indices() {
    fixture(|f| {
        ...
        let out = join::compute_with(&[array], &options, &mut f.builder);
        assert_eq!(out.len(), 1);
...
fn merge_array_shapes_combines_overlapping_keys() {
    fixture(|f| {
        ...
        let out = join::compute_with(&[a, b], &options, &mut f.builder);
        assert_eq!(out.len(), 1);
...
fn merge_array_shapes_off_keeps_separate() {
    fixture(|f| {
        ...
        let a = t_array_with_items(f, &[(key, int)]);
        let b = t_array_with_items(f, &[(key, string)]);
        let out = join::compute_with(&[a, b], &JoinOptions::structural(), &mut f.builder);
        assert_eq!(out.len(), 2);
    });
}
...
fn default_options_match_compute() {
    fixture(|f| {
        ...
        let a = join::compute(&elements, &mut f.builder);
        let b = join::compute_with(&elements, &JoinOptions::default(), &mut f.builder);
        assert_eq!(a, b);
    });
}
