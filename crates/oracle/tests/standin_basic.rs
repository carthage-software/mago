...
fn top_level_template_with_int_constraint_emits_int_standin() {
    fixture(|f| {
        ...
        let result = template::standin(ty, argument, &world, &mut state, &options, &mut f.builder);
...
fn equal_param_and_argument_short_circuits_no_changes() {
    fixture(|f| {
        ...
        let options = StandinOptions::default();
        let result =
            template::standin(well_known::TYPE_INT, well_known::TYPE_INT, &world, &mut state, &options, &mut f.builder);
        assert_eq!(result, well_known::TYPE_INT);
...
fn walk_preserves_existing_declaration_constraint() {
    fixture(|f| {
        ...
        let template = f.t_template("Box", "T");
        let ty = f.u(template);
        template::standin(ty, well_known::TYPE_STRING, &world, &mut state, &options, &mut f.builder);
...
fn multiple_arguments_share_state_and_accumulate_bounds() {
    fixture(|f| {
        ...
        template::standin(parameter, well_known::TYPE_INT, &world, &mut state, &options0, &mut f.builder);
