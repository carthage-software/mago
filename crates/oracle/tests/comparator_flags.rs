...
fn mixed_to_int_sets_coerced_and_from_nested() {
    fixture(|f| {
        ...
        let mixed = f.mixed();
        let int = f.t_int();
        let (verdict, report) = atomic_is_contained_capturing(f, mixed, int, &world);
        assert!(!verdict);
...
fn mixed_to_string_sets_coerced_and_from_nested() {
    fixture(|f| {
        ...
        let (verdict, report) = atomic_is_contained_capturing(f, mixed, string, &world);
        assert!(!verdict);
        assert!(report.coerced());
        assert!(report.causes.contains(CoercionCause::TrueUnionNarrow));
        assert!(report.causes.contains(CoercionCause::NestedMixed));
...
fn array_key_to_int_sets_coerced() {
    fixture(|f| {
        ...
        let int = f.t_int();
        let (verdict, report) = atomic_is_contained_capturing(f, array_key, int, &world);
        assert!(!verdict);
        assert!(report.causes.contains(CoercionCause::TrueUnionNarrow));
        assert!(!report.causes.contains(CoercionCause::NestedMixed));
    });
...
fn array_key_to_string_sets_coerced() {
    fixture(|f| {
        ...
        let array_key = f.t_array_key();
        let string = f.t_string();
        let (verdict, report) = atomic_is_contained_capturing(f, array_key, string, &world);
        assert!(!verdict);
        assert!(report.causes.contains(CoercionCause::TrueUnionNarrow));
    });
}
...
fn object_to_named_sets_coerced_and_object_any_down() {
    fixture(|f| {
        ...
        let (verdict, report) = atomic_is_contained_capturing(f, object, named, &world);
        assert!(!verdict);
        assert!(report.causes.contains(CoercionCause::TrueUnionNarrow));
...
fn bool_to_true_sets_coerced() {
    fixture(|f| {
        ...
        let (verdict, report) = atomic_is_contained_capturing(f, boolean, true_atom, &world);
        assert!(!verdict);
        assert!(report.causes.contains(CoercionCause::TrueUnionNarrow));
    });
}
...
fn bool_to_false_sets_coerced() {
    fixture(|f| {
        ...
        let (verdict, report) = atomic_is_contained_capturing(f, boolean, false_atom, &world);
        assert!(!verdict);
        assert!(report.causes.contains(CoercionCause::TrueUnionNarrow));
    });
}
...
fn lit_int_in_int_no_flags_set() {
    fixture(|f| {
        ...
        let int = f.t_int();
        let (verdict, report) = atomic_is_contained_capturing(f, literal, int, &world);
        assert!(verdict);
        assert!(!report.coerced());
...
fn int_to_lit_int_no_coerced_flag() {
    fixture(|f| {
        ...
        let int = f.t_int();
        let literal = f.t_lit_int(5);
        let (verdict, report) = atomic_is_contained_capturing(f, int, literal, &world);
        assert!(!verdict);
        assert!(!report.coerced());
    });
}
...
fn int_to_positive_int_no_coerced_flag() {
    fixture(|f| {
        ...
        let (verdict, report) = atomic_is_contained_capturing(f, int, positive, &world);
        assert!(!verdict);
        assert!(!report.coerced());
    });
}
...
fn equal_atoms_no_flags() {
    fixture(|f| {
        ...
        for atom in [f.t_int(), f.t_string(), f.t_bool(), f.t_float(), f.null(), f.t_object_any(), f.mixed()] {
            let (verdict, report) = atomic_is_contained_capturing(f, atom, atom, &world);
...
fn never_to_anything_no_flags() {
    fixture(|f| {
        ...
        for atom in [f.t_int(), f.t_string(), f.t_object_any(), f.mixed()] {
            let (verdict, report) = atomic_is_contained_capturing(f, never, atom, &world);
...
fn mixed_to_never_sets_coerced() {
    fixture(|f| {
        ...
        let mixed = f.mixed();
        let never = f.never();
        let (verdict, report) = atomic_is_contained_capturing(f, mixed, never, &world);
        assert!(!verdict);
        assert!(report.causes.contains(CoercionCause::TrueUnionNarrow));
        assert!(report.causes.contains(CoercionCause::NestedMixed));
    });
...
fn concrete_to_never_does_not_set_coerced() {
    fixture(|f| {
        ...
        for atom in [f.t_int(), f.t_string()] {
            let (verdict, report) = atomic_is_contained_capturing(f, atom, never, &world);
            assert!(!verdict);
            assert!(!report.coerced());
        }
    });
...
fn literal_int_to_other_literal_no_coerced_flag() {
    fixture(|f| {
        ...
        let (verdict, report) = atomic_is_contained_capturing(f, one, two, &world);
        assert!(!verdict);
        assert!(!report.coerced());
    });
}
...
fn int_does_not_refine_float_under_strict_subtype() {
    fixture(|f| {
        ...
        let int = f.t_int();
        let float = f.t_float();
        let (verdict, _report) = atomic_is_contained_capturing(f, int, float, &world);
        assert!(!verdict);
    });
}
...
fn template_constrained_to_mixed_sets_from_as_mixed_on_rejection() {
    fixture(|f| {
        ...
        let int = f.t_int();
        let (verdict, report) = atomic_is_contained_capturing(f, template, int, &world);
        assert!(!verdict);
...
fn template_to_mixed_does_not_set_from_as_mixed() {
    fixture(|f| {
        ...
        let template = f.t_template("Foo", "T");
        let mixed = f.mixed();
        let (verdict, report) = atomic_is_contained_capturing(f, template, mixed, &world);
        assert!(verdict);
        assert!(!report.causes.contains(CoercionCause::FromAsMixed));
    });
}
...
fn nullable_int_to_int_with_ignore_null_passes() {
    fixture(|f| {
        ...
        assert!(is_contained_with(f, nullable, int_only, &world, true, false, false));
    });
}
...
fn nullable_int_to_int_without_ignore_null_fails() {
    fixture(|f| {
        ...
        let nullable = f.u_many(vec![int, null]);
        let int_only = f.u(int);
        assert!(!is_contained_with(f, nullable, int_only, &world, false, false, false));
    });
}
