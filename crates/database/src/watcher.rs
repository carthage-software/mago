                ...
                for changed_file in &all_changed {
                    ...
                    let Ok(file) = self.database.get(&changed_file.id) else {
                        if changed_file.path.exists() {
                            ...
                            let host_spec = specificity(&self.host_base_paths);
                            let patch_spec = specificity(&self.patch_base_paths);
                            let include_spec = specificity(&self.include_base_paths);
