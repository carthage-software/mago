+++
title = "Baseline"
description = "为已存在的问题打快照,让 Mago 只标记新出现的问题。提供两种变体,在精度与韧性之间提供不同的取舍。"
nav_order = 20
nav_section = "基础"
+++
# Baseline
...
由更早的 Mago 版本(在引入 variant 支持之前)生成的 baseline 文件没有 `variant` 头。Mago 会把这类文件视为 strict,并打印一条警告,建议你重新生成以获得该头信息。
