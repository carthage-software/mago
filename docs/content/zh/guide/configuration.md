...
- `https://mago.carthage.software/<version>/schema.json`,对应某个具体版本(如 `1.47.2`)。
...
| 选项 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `version` | string | 无 | 锁定本项目所基于的 Mago 版本。接受主版本(`"1"`)、次版本(`"1.47"`)或精确版本(`"1.47.2"`)的锁定。参见 [版本锁定](#version-pinning)。 |
...
- **精确锁定**(`version = "1.47.2"`):任何漂移都会发出警告;跨主版本仍然是致命错误。
