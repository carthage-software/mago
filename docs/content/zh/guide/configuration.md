...
- `https://mago.carthage.software/<version>/schema.json`,对应某个具体版本(如 `1.42.0`)。
...
| 选项 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `version` | string | 无 | 锁定本项目所基于的 Mago 版本。接受主版本(`"1"`)、次版本(`"1.42"`)或精确版本(`"1.42.0"`)的锁定。参见 [版本锁定](#version-pinning)。 |
...
- **次版本锁定**(`version = "1.42"`):任何 `1.42.y` 都满足。漂移到不同的次版本会发出警告;跨主版本仍然是致命错误。
- **精确锁定**(`version = "1.42.0"`):任何漂移都会发出警告;跨主版本仍然是致命错误。
