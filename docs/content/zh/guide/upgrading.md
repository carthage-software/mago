...
对于精确锁定(`version = "1.47.6"`),会直接解析到对应的发布 tag。对于主版本或次版本锁定,Mago 会扫描近期的 GitHub 发布,安装仍然满足锁定的最高版本。所以即便 2.0 已发布,`version = "1"` 仍会安装最新的 1.x。`version = "1.14"` 在 1.19.x 已经流行的情况下,会回退到最新的 1.14.x。
