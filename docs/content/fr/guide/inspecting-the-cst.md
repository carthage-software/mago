+++
title = "Inspecter l'CST"
description = "La commande mago cst pour examiner la sortie du parseur. Un outil de débogage, pas une étape du flux quotidien."
nav_order = 75
nav_section = "Guide"
+++
# Inspecter l'CST
...
| Option | Description |
| :--- | :--- |
| `--tokens` | Affiche le flux de tokens du lexer à la place de l'CST. |
| `--json` | Affiche la sortie (CST ou flux de tokens) en JSON formaté. |
| `--names` | Exécute le résolveur de noms sur l'CST analysé et affiche les noms pleinement qualifiés de chaque symbole. Incompatible avec `--tokens`. |
| `-h`, `--help` | Affiche l'aide et quitte. |

Les options globales doivent précéder `cst`. Voir l'[aperçu CLI](/fundamentals/command-line-interface/) pour la liste complète.
...
- [`mago-syntax`](https://crates.io/crates/mago-syntax) : le lexer, le parseur, les définitions de nœuds CST et des helpers pour parcourir l'arbre.
