...
Surcharge `no-version-check`. Mettez à `true` pour contourner entièrement la vérification de divergence de version, ce qui désactive l'avertissement (ou ignore l'échec) émis quand le binaire installé diverge de la version épinglée dans `mago.toml`. Une divergence de version majeure reste fatale quelle que soit cette variable : tout l'intérêt d'un épinglage majeur est de bloquer les exécutions à travers des schémas de configuration incompatibles.
