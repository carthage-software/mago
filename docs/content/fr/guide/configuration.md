...
- `https://mago.carthage.software/<version>/schema.json`, épinglé à une release précise comme `1.48.0`.
...
```toml
#:schema https://mago.carthage.software/1.48.0/schema.json
version = "1"
php-version = "8.3"
```

```yaml
# yaml-language-server: $schema=https://mago.carthage.software/1.48.0/schema.json
version: "1"
php-version: "8.3"
```
...
| :--- | :--- | :--- | :--- |
| `version` | string | aucun | Fixe la version de Mago contre laquelle ce projet est testé. Accepte un majeur (`"1"`), mineur (`"1.48"`) ou exact (`"1.48.0"`). Voir [épinglage de version](#version-pinning). |
...
- **Épinglage exact** (`version = "1.48.0"`) : toute divergence émet un avertissement ; une divergence majeure reste fatale.
