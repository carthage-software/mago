...
- `https://mago.carthage.software/<version>/schema.json`, épinglé à une release précise comme `1.48.1`.
...
```yaml
# yaml-language-server: $schema=https://mago.carthage.software/1.48.1/schema.json
version: "1"
php-version: "8.3"
```
...
| :--- | :--- | :--- | :--- |
| `version` | string | aucun | Fixe la version de Mago contre laquelle ce projet est testé. Accepte un majeur (`"1"`), mineur (`"1.48"`) ou exact (`"1.48.1"`). Voir [épinglage de version](#version-pinning). |
...
- **Épinglage mineur** (`version = "1.48"`) : tout `1.48.y` satisfait l'épinglage. Une divergence vers un mineur différent émet un avertissement ; une divergence majeure reste fatale.
- **Épinglage exact** (`version = "1.48.1"`) : toute divergence émet un avertissement ; une divergence majeure reste fatale.
...
| `%rel_file%` | Chemin du fichier relatif à l'espace de travail. |
| `%line%` | Numéro de ligne, base 1. |
| `%column%` | Numéro de colonne, base 1. |

Utilisez `%rel_file%` lorsque l'éditeur résout les chemins relativement à l'espace de travail ouvert :
