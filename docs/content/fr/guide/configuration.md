...
- `https://mago.carthage.software/<version>/schema.json`, épinglé à une release précise comme `1.47.3`.
...
```yaml
# yaml-language-server: $schema=https://mago.carthage.software/1.47.3/schema.json
version: "1"
php-version: "8.3"
```
...
| :--- | :--- | :--- | :--- |
| `version` | string | aucun | Fixe la version de Mago contre laquelle ce projet est testé. Accepte un majeur (`"1"`), mineur (`"1.47"`) ou exact (`"1.47.3"`). Voir [épinglage de version](#version-pinning). |
...
- **Épinglage exact** (`version = "1.47.3"`) : toute divergence émet un avertissement ; une divergence majeure reste fatale.
