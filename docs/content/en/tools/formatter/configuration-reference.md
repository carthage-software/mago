...
| `trailing-comma` | boolean | `true` | Add a trailing comma to multi-line arrays, parameter lists, etc. Constructs whose trailing comma the configured `php-version` does not accept never get one: `list()` before 7.2, function calls, `isset()` and `unset()` before 7.3, parameter lists and closure `use` lists before 8.0. |
...
| `preserve-breaking-pipe-expression` | boolean | `false` | Preserve existing line breaks in pipe expressions. |
