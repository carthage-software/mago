...
- `https://mago.carthage.software/<version>/schema.json` — pinned to a specific release such as `1.41.0`.
...
| :--- | :--- | :--- | :--- |
| `version` | string | none | Pins the Mago version this project is tested against. Accepts a major (`"1"`), minor (`"1.41"`), or exact (`"1.41.0"`) pin. See [version pinning](#version-pinning). |
...
- **Minor pin** (`version = "1.41"`): any `1.41.y` satisfies the pin. Drift to a different minor warns; drift across majors is still fatal.
- **Exact pin** (`version = "1.41.0"`): any drift warns; drift across majors is still fatal.
