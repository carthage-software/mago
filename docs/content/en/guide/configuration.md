...
- `https://mago.carthage.software/<version>/schema.json` — pinned to a specific release such as `1.43.0`.
...
| :--- | :--- | :--- | :--- |
| `version` | string | none | Pins the Mago version this project is tested against. Accepts a major (`"1"`), minor (`"1.43"`), or exact (`"1.43.0"`) pin. See [version pinning](#version-pinning). |
...
- **Minor pin** (`version = "1.43"`): any `1.43.y` satisfies the pin. Drift to a different minor warns; drift across majors is still fatal.
- **Exact pin** (`version = "1.43.0"`): any drift warns; drift across majors is still fatal.
