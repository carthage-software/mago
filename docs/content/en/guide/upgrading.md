...
For an exact pin (`version = "1.47.1"`), this resolves directly to that release tag. For a major or minor pin, Mago scans recent GitHub releases and installs the highest one that still satisfies the pin. So `version = "1"` with 2.0 already shipped still installs the latest 1.x release. `version = "1.14"` with 1.19.x in the wild walks back to the latest 1.14.x.
