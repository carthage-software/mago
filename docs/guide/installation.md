...
```sh
curl --proto '=https' --tlsv1.2 -sSf https://carthage.software/mago.sh | bash -s -- --version=1.11.0
```

Or with `wget`:

```sh
wget -qO- https://carthage.software/mago.sh | bash -s -- --version=1.11.0
```
...
```sh
composer require --dev "carthage-software/mago:^1.11.0"
```
