...
```sh
wget -qO- https://carthage.software/mago.sh | bash -s -- --version=1.17.0
```
...
Available tags include `latest`, exact versions (e.g., `1.17.0`), minor versions (`1.17`), and major versions (`1`). Both `linux/amd64` and `linux/arm64` are supported.
...
```sh
composer require --dev "carthage-software/mago:^1.17.0"
```
