...
```sh
wget -qO- https://carthage.software/mago.sh | bash -s -- --version=1.25.1
```
...
Available tags include `latest`, exact versions (e.g., `1.25.1`), minor versions (`1.25`), and major versions (`1`). Both `linux/amd64` and `linux/arm64` are supported.
...
curl --proto '=https' --tlsv1.2 -sSf https://carthage.software/mago.sh \
  | bash -s -- --always-verify --version=1.25.1 --install-dir=/opt/bin
