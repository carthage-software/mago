...
```sh
wget -qO- https://carthage.software/mago.sh | bash -s -- --version=1.22.0
```
...
Available tags include `latest`, exact versions (e.g., `1.22.0`), minor versions (`1.22`), and major versions (`1`). Both `linux/amd64` and `linux/arm64` are supported.
